...
const DEPOSIT_ADDRESS =
    "NRHIUJL9NGMCRYT9BYRLAHAPBVIVIUIPIGWRQKTIBPNRPOOTMEGJHXBASFILYGNQIJOLGGPJMSJMNFWAXSCZVOCCSD";

function promisify<Arg1, Result>(
    fn: (arg1: Arg1, cb: (err: Error, result: Result) => void) => void,
): (arg1: Arg1) => Promise<Result> {
    return (arg1) =>
        new Promise<Result>((resolve, reject) =>
            fn(arg1, (err, result) => (err ? reject(err) : resolve(result))),
        );
}
...
const getLatestInclusion = (hashes: string[]) =>
    new Promise<boolean[]>((resolve, reject) =>
        iota.api.getLatestInclusion(hashes, (err, result) =>
...
const extractAndValidateMessage = async (transaction: ITransactionObject) => {
    ...
    try {
        ...
        const errorMsg = `Error when processing transaction (${transaction.hash.substr(
            0,
            8,
        )}…)`;
        return Promise.reject(errorMsg);
...
function log_transaction_info(transaction: ITransactionObject) {
    ...
    const transactionInfo =
        ` --> Found a ${status} transaction (${startOfHash}…) for ${value} iotas` +
        `issued ${issued} [${issuedLocale}]`;
    console.log(transactionInfo);
}

const checkTransaction = (transactions: ITransactionObject[]) => {
    transactions.filter(isValidTransaction).forEach(async (transaction) => {
        try {
            ...
            console.log(
                " --> User %s successfully granted access",
...
const checkForPayment = async () => {
    ...
    try {
        ...
        console.log(
            " --> %d transactions found for address ",
            transactions.length,
            DEPOSIT_ADDRESS,
        );
        const states = await getLatestInclusion(
            transactions.map((t: any) => t.hash),
        );
        states.forEach((state: boolean, index: number) => {
